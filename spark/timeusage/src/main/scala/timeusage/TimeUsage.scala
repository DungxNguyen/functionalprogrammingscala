...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(List(StructField(columnNames.head, StringType, false)) ++
  ...
  def row(line: List[String]): Row =
    Row.fromSeq(List(line.head) ++ line.tail.map(_.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryStart = List("t01", "t03", "t11", "t1801", "t1803")
    val workingStart = List("t05", "t1805")
    val otherStart = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val primaryNeeds = columnNames.filter(x => primaryStart.exists(x.startsWith(_))).map(new Column(_))
    val workingNeeds = columnNames.filter(x => workingStart.exists(x.startsWith(_))).map(new Column(_))
    val otherNeeds = columnNames.filter(x => (primaryStart ++ workingStart).forall(!x.startsWith(_)) &&
      otherStart.exists(x.startsWith(_))).map(new Column(_))
    (primaryNeeds, workingNeeds, otherNeeds)
  }
  ...
  def timeUsageSummary(
    ...
    otherwise(when(df("teage") >= 23 && df("teage") <= 55, "active").otherwise("elder")).
    as("age")
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(x => TimeUsageRow(x.getAs("working"), x.getAs("sex"), x.getAs("age"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    agg(round(typed.avg[TimeUsageRow](_.other), 1).as[Double], 
        round(typed.avg[TimeUsageRow](_.work), 1).as[Double], 
        round(typed.avg[TimeUsageRow](_.primaryNeeds), 1).as[Double]).
//    agg(round(avg($"other").as[Double], 1).as[Double], 
//        round(avg($"work").as[Double], 1).as[Double], 
//        round(avg($"primaryNeeds").as[Double], 1).as[Double]).
    map(x => TimeUsageRow(x._1._1, x._1._2, x._1._3, x._4, x._3, x._2)).
    orderBy($"working", $"sex", $"age")
  }
}
/**
